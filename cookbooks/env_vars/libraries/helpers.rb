...
module EnvVars
  module Helper
    ...
    def fetch_environment_variables(app_data)
      ...
      return [] unless metadata && metadata['encryption_secret'] && metadata['environment_variables']

      secret = metadata['encryption_secret']
      cipher = MessageCipher.new(secret)
      variables = metadata['environment_variables'].map do |var_hash|
        { :name => var_hash['name'], :value => cipher.decrypt(var_hash['value']) }
      end
    end
